...
> Quick reference for 11 skills + 12 workflow commands
...
## /project:docs
**Purpose:** Generate project documentation structure

```
┌─────────────────────────────────────┐
│  1. Analyze project type            │
│  2. Create docs/ folder structure   │
│  3. Generate templates for:         │
│     • Architecture (ADRs, diagrams) │
│     • API (OpenAPI, guides)         │
│     • Operations (runbooks)         │
│     • Security (threat model)       │
│     • SRE (SLOs, monitoring)        │
│     • User docs (guides, FAQ)       │
│  4. Audit documentation gaps        │
└─────────────────────────────────────┘
```

**Arguments:**
- `init` - Full structure (default)
- `audit` - Check completeness
- `architecture|api|operations|security|sre|user` - Specific category

**Use when:** Project kickoff, before deployment, compliance audits

---

## /project:docs-generate ⭐ NEW
**Purpose:** Auto-generate documentation FROM codebase

```
┌─────────────────────────────────────┐
│  ANALYZES YOUR CODE TO GENERATE:    │
│                                     │
│  Routes/Controllers → API Docs      │
...
**Arguments:**
- `generate` - Generate all docs from code
- `generate api` - API endpoints only
- `generate database` - Schema docs only
- `generate components` - Component library only
- `generate readme` - Update README

**Use when:** After coding, need real documentation from code

---

## /project:implement
**Purpose:** TDD workflow (tests first) + documentation
...
## /project:test-fix
**Purpose:** Auto-fix all failing tests + validate docs
...
## /project:review
**Purpose:** Code review with security + documentation checks
...
## /project:pre-deploy
**Purpose:** Pre-deployment checklist + documentation gate
...
│  ✓ Deployment runbook               │
│  ✓ Rollback runbook                 │
│  ✓ Incident playbook (production)   │
│  ✓ SLO definitions (production)     │
│  ✓ Monitoring docs (production)     │
...
| `/project:docs` | Setup/Deploy | Create doc templates |
| `/project:docs-generate` | Any | Auto-generate docs from code |
| `/project:scaffold` | Coding | Generate boilerplate |
| `/project:implement` | Coding | TDD loop + docs |
| `/project:test-fix` | Testing | Fix all failures + validate docs |
| `/project:review` | Re-coding | Quality + doc check |
| `/project:pre-deploy` | Staging | Checklist + doc gate |
| `/project:post-mortem` | Production | Capture lessons + PIR |
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  1. /project:explore       ─── Understand codebase          │
│           │                                                 │
│           ▼                                                 │
│  2. /project:docs init     ─── Create doc folder structure  │
│           │                                                 │
│           ▼                                                 │
│  3. /project:plan          ─── Plan implementation          │
│           │                                                 │
│           ▼                                                 │
│  4. /project:implement     ─── TDD + update docs            │
│           │                                                 │
│           ▼                                                 │
│  5. /project:test-fix      ─── Verify tests + docs          │
│           │                                                 │
│           ▼                                                 │
│  6. /project:docs-generate ─── Generate docs FROM code      │
│           │                                                 │
│           ▼                                                 │
│  7. /project:review        ─── Code review + doc check      │
│           │                                                 │
│           ▼                                                 │
│  8. /project:docs audit    ─── Verify docs complete         │
│           │                                                 │
│           ▼                                                 │
│  9. /project:pre-deploy    ─── Pre-deployment gate          │
│           │                                                 │
│           ▼                                                 │
│ 10. [DEPLOY]                                                │
│           │                                                 │
│           ▼                                                 │
│ 11. /project:post-mortem   ─── Capture lessons (if issues)  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
```
Pre-deploy  → Tests pass? Migrations ready? Docs complete?
...
> Generated by `/project:docs init`
...
├── sre/
│   ├── slo-definitions.md       # SLI/SLO/SLA
│   ├── monitoring.md            # Metrics & dashboards
│   └── post-incident/           # PIR documents
├── development/
